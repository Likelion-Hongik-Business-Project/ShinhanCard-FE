...
import { searchResultsMockData } from "@/mocks/search/searchMocks";
...
const SearchResultPage = () => {
  ...
  // const { data: searchData, isLoading, error } = useSearchResults(query, 1, 6);

  // Mock 데이터 사용 - 초기값으로 설정
  const [searchData] = useState(searchResultsMockData);

  // 페이지네이션 관련 상태
  const [currentPage, setCurrentPage] = useState(1);
  const ITEMS_PER_PAGE = 6; // 페이지당 6개 아이템
  const totalPages = Math.ceil(searchData.pagination.total / ITEMS_PER_PAGE);
  ...
  const currentItems = searchData.inquiries.slice(startIndex, endIndex);
  ...
  return (
    <section className="w-full h-[835px] bg-gray-10">
      <SearchHeader query={query} total_count={searchData.total_count} />
