...
const MyQuestionsPage = () => {
  // 페이지네이션
  const ITEMS_PER_PAGE = MOCK_MY_QUESTIONS_RESPONSE.pagination.page_size; // 10개
  const totalInquiries = MOCK_MY_QUESTIONS_RESPONSE.total_count; // 총 문의 개수 (ex. 120개)
  const totalPages = Math.ceil(totalInquiries / ITEMS_PER_PAGE); // 총 페이지 (ex. 12개)

  // 팀/필터 관련 상태
  const [selectedTeamId, setSelectedTeamId] = useState(
    MOCK_MY_QUESTIONS_RESPONSE.selected_team.team_id
  );
  ...
  // 현재 페이지 문의들
  const currentItems = MOCK_MY_QUESTIONS_RESPONSE.inquiries
    ...
    <section className="w-full">
      {totalInquiries === 0 ? (
        <div className="h-[calc(100vh-224px)]">
          <h1 className="text-gray-80 text-heading1">내가 쓴 문의</h1>
